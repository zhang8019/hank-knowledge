...
var KnowledgeService = class {
  ...
  async createBase(name2, opts = {}) {
    ...
    return this.deps.store.createBase(name2, {
  ...
  async renameBase(baseId2, name2) {
    return this.deps.store.renameBase(baseId2, name2);
  }
  ...
  async writeImportFile(baseId2, name2, content) {
    const safeName = sanitizeFilename(name2);
    if (!safeName) throw new Error(`\u6587\u4EF6\u540D\u65E0\u6548: ${name2}`);
...
function sanitizeFilename(name2) {
  const cleaned = String(name2 ?? "").replace(/[\\/:*?"<>|\u0000-\u001f]/g, "_").trim().slice(0, 200);
...
var KnowledgeStore = class {
  ...
  async listBases() {
    ...
    for (const name2 of names) {
      ...
      const base = await this.readJson(this.metaPath(name2), null);
  ...
  async renameBase(id, name2) {
    return this.patchBase(id, { name: name2.trim() || "\u672A\u547D\u540D\u77E5\u8BC6\u5E93" });
  }
  // ---- 材料 ----
  ...
  async scanChunks(baseIdValue) {
    ...
    for (const name2 of names) {
      ...
      const file = await this.readJson(join(this.chunksDir(baseIdValue), name2), null);
...
function serviceOf(ctx) {
  return ensureRuntime(ctx).service;
}
function okText(text, details = {}) {
  return {
...
// tools/knowledge_reindex_items.ts
var name = "knowledge_reindex_items";
var description = "\u91CD\u5EFA\u6307\u5B9A\u6750\u6599\uFF08\u6216\u6574\u4E2A\u77E5\u8BC6\u5E93\u5168\u90E8\u6839\u6750\u6599\uFF09\u7684\u7D22\u5F15\u3002\u4EC5\u5141\u8BB8\u5BF9\u5DF2\u5B8C\u7ED3\uFF08completed/failed\uFF09\u7684\u6750\u6599\u6267\u884C\uFF1B\u9002\u7528\u4E8E\u4FEE\u6539\u5185\u5BB9\u540E\u6216 embedding \u914D\u7F6E\u53D8\u5316\u540E\u3002";
var sessionPermission = {
  kind: "routine",
...
async function execute(input, ctx) {
  return guard(async () => {
    ...
    const itemIds = input.itemIds?.length ? input.itemIds : (await service.listItems(input.baseId)).filter((item) => item.parentId === null).map((item) => item.id);
